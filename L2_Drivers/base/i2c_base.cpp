...
I2C_Base::mStateMachineStatus_t I2C_Base::i2cStateMachine()
{
    enum {
        ...
        addressedAsSlaveWrite  = 0x60,
        slaveWriteDataRec      = 0x80,
        stopOrRStartRec        = 0xA0,

        // Slave Transmiter mode
        addressedAsSlaveRead   = 0xA8,
        slaveDataSentAckRec    = 0xB8,
        slaveDataSentNackRec   = 0xC0,
    };
...
#define setStop()           clearSTARTFlag();                           \
    ...
    {
        // Slave Receiver mode
        case addressedAsSlaveWrite:
            first_byte=0;
            setAckFlag();//Data will be received and ACK returned
            clearSIFlag();
            break;

        case slaveWriteDataRec:
            //first time in this state-----store register address sent by

            //after first time
            if(!first_byte)
            {
                slave_base_reg=mpI2CRegs->I2DAT;
                first_byte=1;
            }
            else
            {
                *(buffer_base+(slave_base_reg%(buffer_base_size)))=mpI2CRegs->I2DAT;
                slave_base_reg++;
            }
        ...
        case addressedAsSlaveRead:
            mpI2CRegs->I2DAT=*(buffer_base+(slave_base_reg%buffer_base_size));

            setAckFlag();
            clearSIFlag();
            break;

        case slaveDataSentAckRec:
            slave_base_reg++;
            mpI2CRegs->I2DAT=*(buffer_base+(slave_base_reg%buffer_base_size));
            setAckFlag();
            clearSIFlag();
